...
{   
    ...
    forAll(turbineName,i)
    {
        ...
        if(bladeActuatorPointInterpType[0]== "EVM")
        {
    ...
    // yaw or initial rotor azimuth), the EVM points, the actuator tower points, and the
          ...
          //scalar dist = HubRad[j];
            ...
            for(int m = 0; m < numBladePoints[i]; m++)
            {
             ...
             //bladePointRadius[i][k][m] = dist;
               ...
               totBladePoints++;
               totEVMPoints= totEVMPoints+numEVMPoints;
               dist = dist + 0.5*bladeDs[i][m];
    ...
    forAll(bladesControlled, p)
    {
        ...
        forAll(bladePoints[i], j)
        {
            forAll(bladePoints[i][j], k)
            {
                ...
                else if (bladeActuatorPointInterpType[i] == "EVM")
                {
                    // Use a velocity averaged on a line upstream of the rotor. 
                    #include "velocityInterpolation/EVM.H"
                }
